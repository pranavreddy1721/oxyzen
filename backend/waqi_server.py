"""Render entrypoint that replaces AQI routes with WAQI-safe handlers.

The existing server remains intact for auth, MongoDB, AI, saved locations and
other application routes. These handlers prevent live WAQI snapshots from being
written to the legacy simulated AQI history collection.
"""
...
async def aqi_current_waqi(request: Request, locationId: str = "", lat: float | None = None, lon: float | None = None, locationName: str = "", locationCountry: str = ""):
    ...
    try:
        snap = server.aqi_data.current_snapshot(loc)
    except Exception as exc:
        server.logger.warning("WAQI current request failed: %s", exc)
        raise HTTPException(status_code=502, detail="Live WAQI data is temporarily unavailable for this location.")
...
async def aqi_history_waqi(locationId: str = "", lat: float | None = None, lon: float | None = None, range: str = "24h", locationName: str = "", locationCountry: str = ""):
    loc = _loc(locationId=locationId, lat=lat, lon=lon, locationName=locationName, locationCountry=locationCountry)
    if range not in ("24h", "7d", "30d"):
        range = "24h"
    return {"range": range, "available": False, "points": [], "message": "Historical AQI data is unavailable from the live WAQI API.", "location": {k: loc[k] for k in ("id", "name", "country")}}
