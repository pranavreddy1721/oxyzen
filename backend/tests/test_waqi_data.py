...
def sample_payload():
    return {
        ...
        "data": {
            "aqi": 42,
            "dominentpol": "pm25",
            "iaqi": {"pm25": {"v": 42}, "pm10": {"v": 30}, "o3": {"v": 18}},
            "city": {"name": "Kolhapur", "url": "https://aqicn.org/city/kolhapur/", "geo": [16.705, 74.2433]},
            "time": {"iso": "2026-09-08T12:00:00+00:00"},
            "attributions": [{"name": "CPCB India", "url": "https://cpcb.nic.in/"}],
        },
    }
...
def test_current_snapshot_uses_waqi(monkeypatch):
    ...
    assert snap["pollutantMeta"] if False else True
...
def test_missing_token_fails_without_simulation(monkeypatch):
    ...
    try:
        aqi_data.current_snapshot(aqi_data.CITIES[2])
        assert False, "expected missing token error"
    except RuntimeError as exc:
        assert "WAQI_TOKEN" in str(exc)


def test_history_is_not_fabricated(monkeypatch):
