...
# Verified WAQI station IDs. Sangli's WAQI station is listed as Vijay Nagar,
# Sangli / Hanchinala (A568009); its feed is associated with Sangli but is not
# treated as an exact city-centre measurement.
WAQI_STATIONS = {
    "kolhapur": ["A567994", "A567991"],
    "sangli": ["A568009"],
}
...
def _external_location(x):
    loc={"id":f"geo_{x.get('id')}","name":x.get("name") or "Unknown location","country":x.get("country") or "","admin1":x.get("admin1") or "","lat":float(x["latitude"]),"lon":float(x["longitude"])}
...
def _search_open_meteo(q,limit):
    ...
    try:
        r=requests.get(GEOCODING_URL,params={"name":q,"count":min(limit,20),"language":"en","format":"json"},timeout=4); r.raise_for_status()
        out=[_external_location(x) for x in (r.json().get("results") or [])]; _GEOCODE_CACHE[key]=(now,out); return out[:limit]
    except Exception as exc:
        logger.warning("location search failed: %s",exc); return []

def _same_location(a,b):
    return (a.get("name","").lower(),a.get("country","").lower())==(b.get("name","").lower(),b.get("country","").lower()) or (abs(a["lat"]-b["lat"])<.01 and abs(a["lon"]-b["lon"])<.01)
...
def make_custom_location(lat,lon,name=None,country=""):
    return {"id":f"coord_{round(lat,3)}_{round(lon,3)}","name":name or f"{round(lat,3)}, {round(lon,3)}","country":country,"lat":lat,"lon":lon}
...
def _waqi_json(path,params=None):
    ...
    r=requests.get(f"{WAQI_BASE}{path}",params=p,timeout=8); r.raise_for_status(); payload=r.json()
...
def _distance_km(lat1,lon1,lat2,lon2):
    r=6371.0; p1=math.radians(float(lat1)); p2=math.radians(float(lat2)); dp=math.radians(float(lat2)-float(lat1)); dl=math.radians(float(lon2)-float(lon1))
...
def _fetch_feed(loc,path,fallback_coords=None,enforce_distance=True):
    try:data=_waqi_json(path)
        ...
        logger.warning("WAQI feed %s failed for %s: %s",path,loc.get("name"),exc); return None
    if not _has_usable_aqi(data):
        logger.warning("WAQI feed %s has no current AQI for %s",path,loc.get("name")); return None
    coords=_feed_coordinates(data) or fallback_coords
    if not coords:
        logger.warning("WAQI feed %s has no coordinates for %s",path,loc.get("name")); return None
    distance=_distance_km(loc["lat"],loc["lon"],coords[0],coords[1])
    if enforce_distance and distance>MAX_STATION_DISTANCE_KM:
        logger.warning("Rejected WAQI station %s: %.1f km from %s",(data.get("city") or {}).get("name") or path,distance,loc.get("name")); return None
...
def _resolve_feed(loc):
    ...
    for d,uid,slat,slon in sorted(candidates,key=lambda x:x[0]):
...
def forecast(loc,days=5):
    try:
        ...
        for day,vals in sorted(dates.items())[:max(1,min(7,int(days)))]:
            aqi=int(round(max(vals))); cat=category_for_aqi(aqi)
